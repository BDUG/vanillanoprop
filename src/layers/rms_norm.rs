...
/// Simple 1D parameter with Adam optimiser statistics used by [`RmsNorm`].
pub struct Param {
    pub w: Vec<f32>,
    pub grad: Vec<f32>,
    m: Vec<f32>,
...
impl Param {
    pub fn new(dim: usize, init: f32) -> Self {
        Self {
            w: vec![init; dim],
            grad: vec![0.0; dim],
            m: vec![0.0; dim],
...
pub struct RmsNorm {
    pub gamma: Param,
    eps: f32,
    /// Cached inverse RMS for each row during training.
    inv_rms: Vec<f32>,
    /// Cached input needed for backward pass.
    x: Matrix,
    t: usize,
}
...
impl RmsNorm {
    ...
    pub fn forward(&self, x: &Tensor) -> Tensor {
        ...
        for r in 0..rows {
            ...
            let inv_rms = 1.0 / ((ss / cols as f32) + self.eps).sqrt();
            for c in 0..cols {
                let idx = r * cols + c;
                out.data[idx] = x.data[idx] * inv_rms * self.gamma.w[c];
            }
        }
    ...
    pub fn forward_train(&mut self, x: &Matrix) -> Matrix {
        let rows = x.rows;
        let cols = x.cols;
        self.x = Matrix::zeros(rows, cols);
        self.x.data.clone_from_slice(&x.data);
        ...
        for r in 0..rows {
            ...
            for c in 0..cols {
                let idx = r * cols + c;
                let x_hat = x.data[idx] * inv_rms;
                out.data[idx] = self.gamma.w[c] * x_hat;
            }
        }
        out
    }
    ...
    pub fn backward(&mut self, grad_out: &Matrix) -> Matrix {
        ...
        for r in 0..rows {
            ...
            for c in 0..cols {
                ...
                dot += dy * self.gamma.w[c] * x_val;
                self.gamma.grad[c] += dy * x_val * inv_rms;
            }
            ...
            for c in 0..cols {
                ...
                let dy = grad_out.data[idx];
                let x_val = self.x.data[idx];
                grad_input.data[idx] = inv_rms * self.gamma.w[c] * dy - x_val * dot * inv_rms3 / n;
            }
        }
        grad_input
    }
